...
from src.submit_wg import remove_cookies_popup


class ListingGetter:
    def __init__(self, url: str, page: Optional[Page] = None):
        if page is not None:
            page.goto(url, wait_until="domcontentloaded")
            remove_cookies_popup(page)
            page.locator("#main_column").wait_for(state="attached", timeout=30000)
            html = page.inner_html("#main_column")
        else:
            with sync_playwright() as p:
                browser = p.chromium.launch(headless=True)
                standalone_page = browser.new_page()
                standalone_page.goto(url, wait_until="domcontentloaded")
                html = standalone_page.inner_html("#main_column")
                browser.close()
